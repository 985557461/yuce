 ...
 */
public class ActivityPaiPanResultForName extends ActivityBase implements View.OnClickListener {
	private ImageView back;
	private TextView xMingTextView;
	...
	public static final String kxming = "xming";
	public static final String kliunian = "liunian";
	public static final String knianfen = "nianfen";
	...
	public static void open(Activity activity, String dataStr, String xming, String liunian, String nianfen) {
		Intent intent = new Intent(activity, ActivityPaiPanResultForName.class);
		intent.putExtra(kDataType, "0");
		intent.putExtra(kDataStr, dataStr);
		intent.putExtra(kxming, xming);
		intent.putExtra(kliunian, liunian);
		intent.putExtra(knianfen, nianfen);
	...
	protected void onCreate(Bundle savedInstanceState) {
		if (getIntent() != null) {
			...
			xming = intent.getStringExtra(kxming);
			liunian = intent.getStringExtra(kliunian);
			nianfen = intent.getStringExtra(knianfen);
		}
		setContentView(R.layout.activity_paipan_result_for_name);
	...
	protected void getViews() {
		...
		xMingTextView = (TextView) findViewById(R.id.xMing);
	...
	private void refreshData() {
		...
		nameValuePairs.add(new BasicNameValuePair("xming", xming));
		nameValuePairs.add(new BasicNameValuePair("liunian", liunian));
		nameValuePairs.add(new BasicNameValuePair("nianfen", nianfen));
		Request.doRequest(this, nameValuePairs, ServerConfig.URL_YUCE_XMPAIPAN, Request.POST, new Request.RequestListener() {
			@Override
			...
			public void onComplete(String response) {
				...
				if (model != null) {
					if (!TextUtils.isEmpty(model.xming)) {
						xMingTextView.setText("姓名: " + model.xming);
					}
					...
					if (model.paipanlist != null && model.paipanlist.size() > 0) {
						...
						for (int i = 0; i < count; i++) {
							...
							if (i > 0) {
								...
								textViewParams.bottomMargin = DisplayUtil.dip2px(ActivityPaiPanResultForName.this, 10);
								TextView textView = new TextView(ActivityPaiPanResultForName.this);
							...
							PaiPanResultView view = new PaiPanResultView(ActivityPaiPanResultForName.this);
							view.setListener(new PaiPanResultView.PaiPanResultViewListener() {
								...
								public void onItemClicked(int index) {
									...
									Request.doRequest(ActivityPaiPanResultForName.this, nameValuePairs, ServerConfig.URL_YUCE_XIANGYI, Request.GET, new Request.RequestListener() {
										@Override
										public void onException(Request.RequestException e) {
											dismissDialog();
										...
										public void onComplete(String response) {
											...
											if (detailModel != null && !TextUtils.isEmpty(detailModel.xy)) {
												HCScrollViewDlg.showDlg(ActivityPaiPanResultForName.this, detailModel.xy);
											} else {
												ToastUtil.makeShortText("获取详细信息失败");
											}
										}
							...
							params.bottomMargin = DisplayUtil.dip2px(ActivityPaiPanResultForName.this, 20);
