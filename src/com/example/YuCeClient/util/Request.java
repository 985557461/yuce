...
import com.example.YuCeClient.background.config.AppInfo;
...
public class Request {
	...
	private static RequestParams buildRequestParams(List<NameValuePair> params) {
	...
	public static void doRequestSpecial(Context context, final String url, String method, final RequestListener requestListener) {
		...
		TextHttpResponseHandler textHttpResponseHandler = new TextHttpResponseHandler() {
			...
			public void onFailure(int statusCode, Header[] headers,
					String responseString, Throwable throwable) {
				if (requestListener != null) {
					requestListener.onException(new RequestException(
							responseString));
				}
				if (asyncHttpResponseHandlers.contains(this)) {
					asyncHttpResponseHandlers.remove(this);
		...
		if (POST.equals(method)) {
			asyncHttpClient.get(context, url, requestParams, textHttpResponseHandler);
		} else if (GET.equals(method)) {
			asyncHttpClient.post(context, url, requestParams, textHttpResponseHandler);
		} else {
			asyncHttpClient.post(context, url, requestParams, textHttpResponseHandler);
		}
	}
	...
	public static void doFileUploadRequest(Context context, final List<NameValuePair> params,File file,final String url,final RequestListener listener) {
		RequestParams requestParams = buildRequestParams(params);
		try {
		...
		TextHttpResponseHandler textHttpResponseHandler = new TextHttpResponseHandler() {
			public void onProgress(int bytesWritten, int totalSize) {
				if (listener != null) {
					listener.transferred(1.0f * bytesWritten / totalSize);
				}
				if (asyncHttpResponseHandlers.contains(this)) {
					asyncHttpResponseHandlers.remove(this);
				}
			...
			public void onFailure(int statusCode, Header[] headers,
				...
				if (listener != null) {
					listener.onException(new RequestException(responseBody,
							statusCode));
				}
				if (asyncHttpResponseHandlers.contains(this)) {
					asyncHttpResponseHandlers.remove(this);
