...
public abstract class DragGridView extends GridView {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (isInvalidPosition(mDragPosition)) {
	...
	private void createDragImage(Bitmap bitmap, int downX, int downY) {
		...
		mWindowLayoutParams.format = PixelFormat.TRANSLUCENT; // 图片之外的其TA地方透明
	...
	private Runnable mScrollRunnable = new Runnable() {
		...
		public void run() {
			...
			if (Build.VERSION.SDK_INT >= 11) {
				smoothScrollToPositionFromTop(mDragPosition, view.getTop()
						+ scrollY);
			} else if (Build.VERSION.SDK_INT >= 8) {
				smoothScrollToPosition(mDragPosition);
			}
		}
	};
	...
	private void onSwapItem(int moveX, int moveY) {
		...
		if (isInvalidPosition(tempPosition)) {
			return;
		}
	 ...
	 * @return
	 */
	abstract boolean isInvalidPosition(int position);
}